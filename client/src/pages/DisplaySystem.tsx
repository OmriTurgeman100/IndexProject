...
import {
  get_specific_system,
  get_system_virtual_machines,
  get_system_dependencies,
  get_system_authentication,
  create_dependency_for_system,
  create_virtual_machine_for_system,
  create_system_authentication,
  // update_virtual_machine,
  update_virtual_machine_total,
  update_dependency,
  update_authentication,
  get_system_dep_info,
  post_system_dep_info,
  patch_system_dep_info,
  delete_system_dep_info,
  delete_system_dep,
  delete_system_vm,
  delete_system_auth,
  get_system_core_dependencies,
  post_system_core_dependency,
  delete_system_core_dependency,
  get_system_links,
  delete_system_link,
  update_system_link,
  create_system_link,
...
import { update_system, update_system_total } from "../services/System";
...
import { get_services_menu } from "../services/Get_Services_Menu";
...
// import HubIcon from "@mui/icons-material/Hub"; // For core_network (network core)
...
import { SystemExcelServers } from "../components/SystemExcelServers";
import DriveFolderUploadIcon from "@mui/icons-material/DriveFolderUpload";
import StarIcon from "@mui/icons-material/Star";
import { dependency_auto_suggestions, authentication_auto_suggestions } from "../services/Get_Visualization_Total";
...
interface DepInfoSystem {
  id: number;
  system_parent: number;
  system_dep_parent: number;
  description: string;
}
...
interface system_docs {
  id: number;
  system_id: number;
  title: number;
}

interface system_sites {
  site: string;
}
...
interface authentication_suggestions {
  authentication_type: string
}
...
export const DisplaySystem = () => {
  ...
  const [authList, setAuthList] = useState<AuthInterface[]>([]);
  const [editSystemInfo, setEditSystemInfo] = useState<boolean>(false);
  const [FormPlaceHolder, setFormPlaceHolder] = useState<string>("");
  const [EditSystemInfoCore, setEditSystemInfoCore] = useState(false);
  ...
  const [CreateAuth, setCreateAuth] = useState<boolean>(false);
  ...
  const [SystemDepForm, setSystemDepForm] = useState<string>("");
  const [SystemAuthForm, setSystemAuthForm] = useState<string>("");
  ...
  const [depInfoData, setInfoData] = useState<DepInfoSystem[]>([]);
  ...
  const [coreDeps, setCoreDeps] = useState<CoreDependencies[]>([]);
  const [postCoreDep, setPostCoreDep] = useState<boolean>(false);
  const [servicesMenu, setServicesMenu] = useState<service_menu[]>([]);
  const [mode, setMode] = useState<string>("dep");
  const [viewMode, setViewMode] = useState<string>("old");
  const [systemLinks, setSystemLinks] = useState<system_links[]>([]);
  ...
  const [docsList, setDocsList] = useState<system_docs[]>([]);
  ...
  const [systemSites, setSystemSites] = useState<system_sites[]>([]);
  const [dependencySuggestions, setDependencySuggestions] = useState<dependency_suggestions[]>([])
  const [authenticationSuggestions, setAuthenticationSuggestions] = useState<authentication_suggestions[]>([])
  const navigate = useNavigate();
  const { theme } = useThemeContext();

  const fetch_data = async () => {
    try {
      const response = await get_specific_system(id);
      setData(response.data);

      const virtual_machines = await get_system_virtual_machines(id);

      setVMList(virtual_machines.data);

      const dep = await get_system_dependencies(id);

      setDepList(dep.data);

      const auth = await get_system_authentication(id);

      setAuthList(auth.data);

      const core_dependencies = await get_system_core_dependencies(id);

      setCoreDeps(core_dependencies.data);

      const service_menu = await get_services_menu();

      setServicesMenu(service_menu.data);

      const system_links = await get_system_links(id);

      setSystemLinks(system_links.data);

      const system_docs = await display_system_docs(id);

      setDocsList(system_docs.data);

      const system_sites = await display_system_sites(id);

      setSystemSites(system_sites.data);
  ...
  const handle_dep = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      setSystemDepForm(event.target.value);
  ...
  const handle_auth = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      setSystemAuthForm(event.target.value);

      const response = await authentication_auto_suggestions(event.target.value)

      setAuthenticationSuggestions(response.data)
  ...
  async function handle_server_create() {
    try {
      await create_virtual_machine_for_system(
        id,
        ServerNameForm,
        ServerSiteForm,
        ServerNetworkForm,
  ...
  async function handle_dep_create() {
    try {
      await create_dependency_for_system(id, SystemDepForm);
  ...
  async function handle_auth_create() {
    try {
      await create_system_authentication(id, SystemAuthForm);
  ...
  const handle_submit = async () => {
    try {
      await update_system(id, FormPlaceHolder, FormData);

      setEditSystemInfo(false);
    } catch (error) {
      console.error(error);
    }
  };

  const handle_system_edit = async (
    impact: string,
    masad: string,
    secondary_site: string,
    backup_type: string,
  ...
  //   try {
  //     await update_system(id, "core_network", networkForm);
  ...
  //   try {
  //     await update_system(id, "owned_by", teamForm);
  ...
  //   try {
  //     await update_system(id, "system_name", nameForm);
  ...
  async function StartEditDep(dep_id: any, dependency: string) {
    ...
    setSystemDepForm(dependency);
    setEditDep(true);
  }

  async function StartEditAuth(auth_id: any, auth: string) {
    setAuthId(auth_id);

    setSystemAuthForm(auth);
  ...
  async function handle_dep_post_edit(): Promise<void> {
    try {
      await update_dependency(depId, SystemDepForm);
  ...
  async function handle_auth_post_edit(): Promise<void> {
    try {
      await update_authentication(authId, SystemAuthForm);
  ...
  async function display_dep(id: number): Promise<void> {
    try {
      const response = await get_system_dep_info(id);
  ...
  const sumbit_dep_info_edit = async () => {
    try {
      await patch_system_dep_info(depInfoId, depInfoEditForm);
  ...
  const sumbit_dep_info_post = async () => {
    try {
      await post_system_dep_info(id, ParentDepId, depInfoPostForm);
  ...
  const delete_dep_info = async (id: string | number | undefined) => {
    try {
      await delete_system_dep_info(id);
  ...
  const delete_auth = async (id: string | number | undefined) => {
    try {
      await delete_system_auth(id);
      setRefreshDelete((prev) => !prev);
    } catch (error) {
      console.error(error);
    }
  ...
  const delete_core_dependency = async (id: string | number | undefined) => {
    try {
      await delete_system_core_dependency(id);
  ...
  const link_service_to_system = async (
    ...
    try {
      switch (mode) {
        case "dep":
          await create_dependency_for_system(id, service_name);
          break;
        case "auth":
          await create_system_authentication(id, service_name);
          break;
        default:
          break;
      }

      await post_system_core_dependency(id, service_id);
  ...
  const handle_delete_link = async (link_id: number) => {
    try {
      await delete_system_link(link_id);
  ...
  const handle_edit_link_submit = async () => {
    try {
      await update_system_link(LinkId, LinkTitleForm, LinkUrlForm);
  ...
  const handle_post_link_submit = async () => {
    try {
      await create_system_link(id, LinkTitleForm, LinkUrlForm);
  ...
  const handle_post_doc_submit = async () => {
    try {
      await create_system_doc(id, DocTitleForm);
  ...
  const handle_selected_site_edit = async (event: SelectChangeEvent) => {
    try {
      await update_system(id, "preferred_site", event.target.value);
  ...
  const handle_selected_impact_edit = async (event: SelectChangeEvent) => {
    try {
      await update_system(id, "impact", event.target.value);
  ...
  const handle_selected_backup_edit = async (event: SelectChangeEvent) => {
    try {
      await update_system(id, "backup_type", event.target.value);
      ...
      {loading === false ? (
        ...
        >
            ...
            >
              ...
              {EditSystemInfoCore === false ? (
                ...
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    <IconButton onClick={() => navigate(`/tree/${id}/system`)}>
                      <AccountTreeIcon
                        sx={{ color: theme.palette.text.primary }}
                      />
                    </IconButton>
              ...
              {EditSystemInfoCore === false ? (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Typography sx={{ fontSize: "20" }}>צוות מטפל</Typography>
                ...
                >
                  ...
                  <IconButton
                    ...
                    onClick={() => setEditSystemInfoCore(false)}
                  >
                ...
                <>
                  {ShowDescInput === false ? (
                    <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                      ...
                      <IconButton
                        sx={{ backgroundColor: theme.palette.secondary.main }}
                        onClick={() => setEditSystemInfo(false)}
                      >
                        <CloseIcon sx={{ color: theme.palette.text.primary }} />
                      </IconButton>
                    ...
                    >
                      {dependencySuggestions.map((item, index) => (
                        <Box
                          key={index}
                          onClick={() => {
                            setSystemDepForm(item.dependency);
                  ...
                  <Box>
                    ...
                    >
                      ...
                      <TextField
                        ...
                        variant="outlined"
                        value={SystemAuthForm}
                        onChange={handle_auth}
                        placeholder="סוג הזדהות של המערכת "
                      />
                      <Typography>סוג הזדהות</Typography>
                    </Box>

                    {authentication_auto_suggestions.length > 0 && (
                      <Box
                        sx={{
                          maxHeight: "300px",
                          height: "fit-content",
                      ...
                      >
                        {authenticationSuggestions.map((item, index) => (
                          <Box
                            key={index}
                            onClick={() => {
                              setSystemAuthForm(item.authentication_type);
                              setAuthenticationSuggestions([]);
                            }}
                            sx={{
                              px: 2,
                              py: 1,
                  ...
                  {authList && authList.length > 0 ? (
                    authList.map((item) => (
                      ...
                      >
                        ...
                        <IconButton
                          onClick={() =>
                            StartEditAuth(item.id, item.authentication_type)
                          }
                        >
                          <EditIcon
                            sx={{ color: theme.palette.text.primary }}
            ...
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <TextField
                  ...
                  placeholder="auth"
                />
                <Typography>ערוך הזדהות</Typography>
              </Box>
              {authenticationSuggestions.map((item, index) => (
                <Box
                  key={index}
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            ...
            <Typography>קישוריות ליבה</Typography>
          </Box>
          {coreDeps && coreDeps.length > 0 ? (
            <Box
              sx={{
                width: "1000px",
                // backgroundColor: theme.palette.primary.main,
            ...
            >
              {coreDeps.map((item) => (
                ...
                >
                  <Link to={`/services/${item.service_id}`}>
                    {item.service_name}
                  ...
                  <IconButton
                    onClick={() => delete_core_dependency(item.id)}
                    sx={{ position: "absolute", right: 0 }}
                  >
                    <DeleteIcon />
                  </IconButton>
          ...
          >
            <VirtualMachinesUsageBar type="system" id={id} />
          </Paper>
          <Dialog open={postCoreDep} onClose={CloseCoreDep}>
            <Box sx={{ display: "flex", alignItems: "center", padding: 1 }}>
              ...
              <DialogTitle>אנא בחר שירות</DialogTitle>
            </Box>
            ...
            >
              {servicesMenu.map((item) => (
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  ...
                  <IconButton
                    onClick={() =>
                      link_service_to_system(item.service_id, item.service_name)
                    }
                    sx={{ backgroundColor: theme.palette.primary.main }}
                  >
                    <AddIcon />
          ...
          <Dialog open={showDocs} onClose={() => setShowDocs(false)}>
            ...
            >
              ...
              <DialogTitle>מסמכי המערכת </DialogTitle>
            </Box>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
                {docsList.map((item) => (
                  <Box
                    ...
                    onClick={() => navigate(`/docs/${item.id}/?type=system`)}
                  >
