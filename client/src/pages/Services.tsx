...
import { create_service_with_name } from "../services/Service";
...
export const Services = () => {
  const [menu, setMenu] = useState<service_menu[]>([]);
  ...
  const fetch_data = async () => {
    try {
      const response = await get_services_menu();
  ...
  const handle_submit = () => {
    const result = menu.find(({ service_name }) =>
      service_name.toLowerCase().includes(formData.toLowerCase())
    );

    const id: number | undefined = result?.service_id;

    navigate(`/services/${id}`);
  };

  const handle_menu_submit = (item_id: number) => {
    navigate(`/services/${item_id}`);
  };
  ...
  function handle_post_service(): void {
  ...
  const post_service = async () => {
    try {
      await create_service_with_name(formData);
        ...
        >
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              >
                שלום, כאן ניתן לחפש מידע על השירות שלכם
              </Typography>
            </Box>
          ...
          >
            {menu.map((item) => (
              ...
              >
                ...
                <IconButton
                  sx={{ backgroundColor: theme.palette.primary.main }}
                  onClick={() => handle_menu_submit(item.service_id)}
                >
                  <ArrowDropUpIcon />
                </IconButton>
              </Paper>
        ...
        >
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              ...
              >
                שלום, כאן ניתן להוסיף את השירות שלכם
              </Typography>
            </Box>
            <Typography sx={{ textAlign: "center", fontSize: "15px" }}>
              אנא כתוב את שם השירות
            </Typography>
            <Box
              sx={{
                width: "fit-content",
